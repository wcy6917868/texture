...
- (void)configUI
{
    ...
    UILabel *groupInvite = [[UILabel alloc]initWithFrame:CGRectMake(164*SCREENW_RATE, CGRectGetMaxY(lineV.frame)+20*SCREENW_RATE, 60*SCREENW_RATE, 15*SCREENW_RATE)];
    groupInvite.font = [UIFont systemFontOfSize:12];
    groupInvite.text = @"群友邀请";
    [self.view insertSubview:groupInvite aboveSubview:bigWhiteBg];
    
    UIView *lineV1 = [[UIView alloc]initWithFrame:CGRectMake(173*SCREENW_RATE, CGRectGetMaxY(groupInvite.frame)+10*SCREENW_RATE, 30*SCREENW_RATE, 1*SCREENW_RATE)];
    ...
    for (int i = 0; i <= 10; i ++)
    {
        NSInteger X = i%7;
        NSInteger Y = i/7;
        UIView *blueV  = [[UIView alloc]initWithFrame:CGRectMake(12+52*X, CGRectGetMaxY(lineV1.frame)+12+52*Y, 40*SCREENW_RATE, 40*SCREENW_RATE)];
        blueV.backgroundColor = RGB(53, 195, 236);
        blueV.layer.cornerRadius = 5.0f;
        blueV.layer.masksToBounds = YES;
        if (i == 10)
        {
            blueV.backgroundColor = [UIColor whiteColor];
            blueV.layer.borderColor = RGB(153, 153, 153).CGColor;
            blueV.layer.borderWidth = 0.3f;
            UIImageView *addImg = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, 20*SCREENW_RATE, 20*SCREENW_RATE)];
            addImg.image = [UIImage imageNamed:@"+"];
            addImg.center = CGPointMake(CGRectGetMidX(blueV.frame), CGRectGetMidY(blueV.frame));
            [blueV addSubview:addImg];
        }
