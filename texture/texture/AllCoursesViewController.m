...
@interface AllCoursesViewController ()<UITextFieldDelegate,UITableViewDataSource,UITableViewDelegate>
{
...
- (void)configNav
{
    ...
    self.navigationItem.title = @"全部课堂";
    [self.navigationController.navigationBar setTitleTextAttributes:
     @{NSFontAttributeName:[UIFont systemFontOfSize:18],
       NSForegroundColorAttributeName:RGB(51, 51, 51)}];
    
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc]initWithTitle:@"+ 发布" style:UIBarButtonItemStyleDone target:self action:@selector(create)];
...
- (void)configUI
{
    ...
    for (int i = 0; i < 5; i ++)
    {
        ...
        [tableViewBtn setTitle:@"热门课堂" forState:UIControlStateNormal];
...
- (UITableView *)tableV
{
    if (!_tableV)
    {
        _tableV = [[UITableView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(scrollView.frame), SCREENW, SCREENH - CGRectGetMaxY(scrollView.frame)) style:UITableViewStylePlain];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!cell)
    {
        cell = [[CourseCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellID];
    }
