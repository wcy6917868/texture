...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier])
    {
        ...
        UIImageView *admireImg = [[UIImageView alloc]initWithFrame:CGRectMake(313*SCREENW_RATE, 67*SCREENW_RATE, 10*SCREENW_RATE, 10*SCREENW_RATE)];
        admireImg.image = [UIImage imageNamed:@"dianzan0"];
        [self.contentView addSubview:admireImg];
        
        _admireNumL = [[UILabel alloc]initWithFrame:CGRectMake(CGRectGetMaxX(admireImg.frame)+9*SCREENW_RATE, admireImg.frame.origin.y + 2*SCREENW_RATE, 40*SCREENW_RATE, 10*SCREENW_RATE)];
        _admireNumL.font = [UIFont systemFontOfSize:10];
        _admireNumL.textColor = RGB(102, 102, 102);
        _admireNumL.text = @"9999+";
        _admireNumL.textAlignment = NSTextAlignmentLeft;
        [self.contentView addSubview:_admireNumL];
