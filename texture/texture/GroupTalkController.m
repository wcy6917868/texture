...
@interface GroupTalkController ()

@end

@implementation GroupTalkController
...
- (void)configUI
{
    ...
    [groupName setAttributedPlaceholder:[[NSAttributedString alloc]initWithString:@"填写群聊名称" attributes:@{NSForegroundColorAttributeName:RGB(153, 153, 153)}]];
    [self.view insertSubview:groupName aboveSubview:bigWhiteBg];
    
    UIView *lineV = [[UIView alloc]initWithFrame:CGRectMake(138*SCREENW_RATE, CGRectGetMaxY(groupName.frame)+7*SCREENW_RATE, 100*SCREENW_RATE, 1)];
    ...
    [groupIntro setAttributedPlaceholder:[[NSAttributedString alloc]initWithString:@"群友邀请" attributes:@{NSForegroundColorAttributeName:RGB(51, 51, 51)}]];
    ...
    for (int i = 0; i < 13 ; i ++)
    {
        NSInteger X = i % 7;
        NSInteger Y = i / 7;
        UIImageView *headImg = [[UIImageView alloc]initWithFrame:CGRectMake((12 + X * 52) *SCREENW_RATE, CGRectGetMaxY(lineV1.frame)+12*SCREENW_RATE + (Y * 52)*SCREENW_RATE, 40*SCREENW_RATE, 40*SCREENW_RATE)];
        headImg.backgroundColor = RGB(53, 195, 236);
        headImg.layer.cornerRadius = 5.0f;
        headImg.layer.masksToBounds = YES;
        headImg.tag = 100 + i;
        if (i == 12) {
            headImg.image = [UIImage imageNamed:@"tianjiagengduo0"];
            headImg.backgroundColor = [UIColor whiteColor];
        }
        [self.view insertSubview:headImg aboveSubview:bigWhiteBg];
    }
